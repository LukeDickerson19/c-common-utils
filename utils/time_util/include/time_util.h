...
/* sleep_microseconds() pauses the current thread for the specified duration
    - microseconds: duration to sleep in microseconds

    Notes:
    - Uses nanosleep() on POSIX systems and Sleep() on Windows
    - Actual sleep duration may be slightly longer due to OS scheduling
    - Intended for timing, throttling, and test delays
*/
